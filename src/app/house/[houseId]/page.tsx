...
export default function HouseDashboard() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (userData.role === "admin" || userData.houseId === houseId) {
          ...
          alert(
            "Access denied. You don't have permission to access this house."
          );
          window.location.href = "/login";
        }
      } else {
      ...
      if (roundsRes.ok) {
        ...
        if (activeRoundData) {
          ...
          if (participantsRes.ok) {
            ...
            const participant = Array.isArray(participantsData)
              ? participantsData.find(
                  (p: Participant) =>
                    p._id?.toString() ===
                    activeRoundData.participantId?.toString()
                )
              : null;
          ...
          const bidsRes = await fetch(
            `/api/bids?participantID=${activeRoundData.participantId}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const placeBid = async () => {
    ...
    setTimeout(() => {
      setHouse({
        ...house,
        remainingBudget: house.remainingBudget - bidAmount,
      });
      setBidAmount(0);
      setHasBid(true);
      setLoading(false);
