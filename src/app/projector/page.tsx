  ...
  useEffect(() => {
    if (currentState?.screen === "bidding" && currentState.timerEnd) {
      const interval = setInterval(() => {
        const remaining =
          new Date(currentState.timerEnd).getTime() - Date.now();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-800 to-gray-900 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="mt-8">
            <div
              className={`inline-block px-4 py-2 rounded-full ${isConnected ? "bg-green-500" : "bg-red-500"}`}
            >
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-900 to-purple-900 text-white">
        ...
        <div className="max-w-7xl mx-auto p-8">
          ...
          <div className="text-center mb-12">
            <div
              className={`text-9xl font-bold mb-4 ${isTimeRunningOut ? "text-red-400 animate-pulse" : "text-white"}`}
            >
              {formatTime(timeLeft)}
            </div>
              ...
              <div className="space-y-4">
                ...
                {housesWithBids.length > 0 ? (
                  <div className="grid grid-cols-2 gap-4">
                    {houses
                      .filter((house) =>
                        housesWithBids.includes(house._id?.toString() || "")
                      )
                      .map((house) => (
                        <div
                ...
                {houses.length > housesWithBids.length && (
                  <div className="mt-8">
                    <div className="text-xl mb-4 text-gray-300 text-center">
                      Waiting for:
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      {houses
            ...
            <div className="space-y-8">
              ...
              <div className="bg-yellow-500 bg-opacity-20 rounded-2xl p-8 border-4 border-yellow-400">
                ...
                <div className="text-6xl font-bold text-yellow-400 mb-4">
                  {houses.find((h) => h._id?.toString() === winner.houseId)
                    ?.name || "Unknown House"}
                </div>
                    ...
                    {losers.map((loser: any, index: number) => (
                      ...
                      >
                        <div className="text-2xl font-semibold mb-2">
                          {houses.find(
                            (h) => h._id?.toString() === loser.houseId
                          )?.name || "Unknown"}
                        </div>
