...
describe("Bidding System - Validation & Constraints", () => {
  async function setup() {
    ...
    const house = await Houses.create({ name: `H-${Date.now()}`, totalBudget: 500, remainingBudget: 500 });
    ...
    const participant = await Participants.create({ name: "Test", rollNumber: `25K-${Date.now()}`, teamId: new ObjectId(team.insertedId.toString()) });
    const round = await Rounds.create({ participantId: new ObjectId(participant.insertedId.toString()), status: "active", timerEnd: new Date(Date.now() + 10000), finalized: false });

    return { token, houseId, roundId: round.insertedId.toString(), participantId: participant.insertedId.toString() };
  }
  ...
  it("rejects bid on non-active round", async () => {
    ...
    const team = await Teams.create({ rank: 1 });
    const participant = await Participants.create({ name: "Test", rollNumber: `25K-${Date.now()}`, teamId: new ObjectId(team.insertedId.toString()) });
    const round = await Rounds.create({ participantId: new ObjectId(participant.insertedId.toString()), status: "scheduled", timerEnd: null, finalized: false });
  ...
  it("rejects bid after timer expired", async () => {
    ...
    const team = await Teams.create({ rank: 1 });
    const participant = await Participants.create({ name: "Test", rollNumber: `25K-${Date.now()}`, teamId: new ObjectId(team.insertedId.toString()) });
    const round = await Rounds.create({ participantId: new ObjectId(participant.insertedId.toString()), status: "active", timerEnd: new Date(Date.now() - 1000), finalized: false });
  ...
  it("enforces 3-per-batch cap", async () => {
    ...
    const team = await Teams.create({ rank: 1 });
    for (let i = 0; i < 3; i++) {
      await Participants.create({ name: `P${i}`, rollNumber: `25K-${Date.now()}-${i}`, teamId: new ObjectId(team.insertedId.toString()), houseId: new ObjectId(houseId) });
    }

    // Try 4th
    const p4 = await Participants.create({ name: "P4", rollNumber: `25K-${Date.now()}-4`, teamId: new ObjectId(team.insertedId.toString()) });
    const round = await Rounds.create({ participantId: new ObjectId(p4.insertedId.toString()), status: "active", timerEnd: new Date(Date.now() + 10000), finalized: false });
  ...
  it("groups seniors across years (max 3 total)", async () => {
    ...
    const house = await Houses.create({ name: `H-${Date.now()}`, totalBudget: 5000, remainingBudget: 5000 });
    const houseId = house.insertedId.toString();
    const user = await Users.create({ username: `cap-${Date.now()}`, password: "hash", role: "house_captain", houseId: new ObjectId(houseId) });
    const token = generateToken({ userId: user.insertedId.toString(), username: "cap", role: "house_captain", houseId });
    ...
    for (const year of ["22", "21", "20"]) {
      await Participants.create({ name: `S${year}`, rollNumber: `${year}K-${Date.now()}`, teamId: new ObjectId(team.insertedId.toString()), houseId: new ObjectId(houseId) });
    }

    // Try 4th senior from year 19
    const s4 = await Participants.create({ name: "S19", rollNumber: `19K-${Date.now()}`, teamId: new ObjectId(team.insertedId.toString()) });
    const round = await Rounds.create({ participantId: new ObjectId(s4.insertedId.toString()), status: "active", timerEnd: new Date(Date.now() + 10000), finalized: false });
