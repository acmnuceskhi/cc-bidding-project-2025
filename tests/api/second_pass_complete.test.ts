...
describe("Pass 2 - Second Chance Bidding", () => {
  ...
  it("allows houses below minimum roster in pass 2", async () => {
    ...
    for (let i = 0; i < 2; i++) {
      await Participants.create({
        name: `P25-${i}`,
        rollNumber: `25K-${Date.now()}-${i}`,
        teamId: new ObjectId(team1.insertedId.toString()),
        houseId: new ObjectId(houseId),
      });
    }
    for (let i = 0; i < 2; i++) {
  ...
  it("pass 2 winners count toward batch cap", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Assign winner
      await Participants.update(p.insertedId.toString(), {
        houseId: new ObjectId(houseId),
      });
    }
  ...
  it("houses can participate in both passes for different participants", async () => {
    ...
    const p1 = await Participants.create({
      name: "Pass1",
      rollNumber: `25K-${Date.now()}-p1`,
      teamId: new ObjectId(team1.insertedId.toString()),
    });
    const r1 = await Rounds.create({
      participantId: new ObjectId(p1.insertedId.toString()),
      status: "active",
      timerEnd: new Date(Date.now() + 10000),
      finalized: false,
      passPhase: 1,
