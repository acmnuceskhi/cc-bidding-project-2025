...
describe("Unsold participants list edge case", () => {
  ...
  async function createHouse(name: string) {
    ...
    const res = await Houses.create({ name, totalBudget: 1000, remainingBudget: 1000 });
  ...
  async function createParticipant(label: string) {
    ...
    const team = await Teams.create({ successfulAttempts: 1, unsuccessfulAttempts: 0, totalPoints: 100, totalPenalty: 0, timeTakenPerProblem: [10,10,10,10,10], rank: 1 });
  ...
  async function createRoundForParticipant(participantId: string, status: "scheduled"|"completed"|"active", passPhase: 1) {
    const { Rounds } = await import("@/lib/models/rounds");
    const { ObjectId } = await import("mongodb");
    const res = await Rounds.create({ participantId: new ObjectId(participantId), status, passPhase, timerEnd: null });
    return res.insertedId.toString();
  }

  async function markRoundSkipped(roundId: string) {
    const { Rounds } = await import("@/lib/models/rounds");
    await Rounds.update(roundId, { status: "completed", skipped: true, finalized: false });
  }

  async function markRoundSold(roundId: string, participantId: string, houseId: string) {
    const { Rounds } = await import("@/lib/models/rounds");
    const { Participants } = await import("@/lib/models/participants");
    const { ObjectId } = await import("mongodb");
    await Rounds.update(roundId, { status: "completed", skipped: false, finalized: true, winningBid: 100 });
    await Participants.update(participantId, { houseId: new ObjectId(houseId) });
  }
  ...
  it("only includes skipped pass-1 participants without houseId in unsold list", async () => {
    ...
    const rScheduled = await createRoundForParticipant(pScheduled, "scheduled", 1);
    ...
    const unsoldIds = status.unsoldParticipants.map((p: { participantId: string }) => p.participantId);
